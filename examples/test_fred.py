...
class TestApiMethod(unittest.TestCase):
    ...
    def test_new_api_key_for_api_called_with_series_updates(self):
        Fred('new_api_key').call_api('series/updates', filter_value='regional')
        results = called_url()
    ...
    def test_api_with_no_output_formatting(self):
        Fred().call_api('release/dates', output_format=None)
        results = called_url()
        self.assertEqual(results['path'], '/fred/release/dates')
        self.assertFalse(api.xml2dict.called)
        self.assertFalse('output_format' in results['query'])
